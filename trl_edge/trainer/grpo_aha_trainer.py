...
class GRPOAhaTrainer(Trainer):
    ...
    def __init__(
        ...
        if args.reward_weights is not None:
            ...
            self.reward_weights = torch.tensor(args.reward_weights, dtype=self.dtype)
        else:
            self.reward_weights = torch.ones(len(reward_funcs), dtype=self.dtype)
    ...
    def first_calculate_rewards(self, inputs, prompts, completions, completion_ids_list):
        device = self.accelerator.device
        rewards_per_func = torch.zeros(len(prompts), len(self.reward_funcs), device=device)
    ...
    def _generate_and_score_completions(
        ...
        #prompts_text_withtokens = prompts_text
        #print("len(prompts_text):",len(prompts_text))
        print("prompts_text[0]:",prompts_text[0])
        ...
        with unwrap_model_for_generation(self.model_wrapped, self.accelerator, gather_deepspeed3_params=self.args.ds3_gather_for_generation) as unwrapped_model:
            with (
                ...
                # "do_sample": true, "eos_token_id": 151643, "max_new_tokens": 1024, "pad_token_id": 151643, "temperature": 0.6
                self.generation_config.max_new_tokens = 1024
                prompt_completion_ids = unwrapped_model.generate(
                    prompt_ids, attention_mask=prompt_mask, generation_config=self.generation_config) #[num_generations, prompt_length + max_completion_lengths]
        ...
        with torch.no_grad():
            if self.force_reflection or self.GEC:
                ...
                regenerate_indices = torch.where(rewards != 2)[0]
                if prompt_length < 256:
                    print(f"Regenerating {len(regenerate_indices)} completions with reward!=2")
                    
                    # Prepare new prompts for regeneration
                    need_to_gen_ind = []
                        ...
                        if idx in regenerate_indices:
                            ...
                            if self.GEC:
                                ...
                                if selected == aha_begin:
                                    ...
                                    prompts_text[idx] = prompts_text[idx] + completions_text[idx]
                                    need_to_gen_ind.append(idx) ###need to generate again
                                else:
                                    give_answer = "\nWait! Something is wrong here. After verification, the correct answer is \\boxed{" + answers[idx] + "}."
                                    selected = random.choice([give_answer, solution[idx]])
                                    if selected == give_answer:
                                        completions_text[idx] = completions_text[idx] + give_answer
                                    else:
                                        completions_text[idx] = solution[idx]
                                    prompts_text[idx] = prompts_text[idx] + completions_text[idx]
                            else:
                                completions_text[idx] = completions_text[idx] + aha_begin
                                prompts_text[idx] = prompts_text[idx] + completions_text[idx]
                    ...
                    prompt_inputs = self.processing_class(prompts_text, return_tensors="pt", padding=True, padding_side="left", add_special_tokens=False).to(device)
                    new_prompt_ids = prompt_inputs.input_ids
                    new_prompt_mask = prompt_inputs.attention_mask
                    ...
                    for idx in range(num_gen):
                        if idx in need_to_gen_ind:
                            completions_text[idx] = completions_text[idx] + new_completions_text[idx]
                            prompts_text[idx] = prompts_text[idx] + new_completions_text[idx]
    ...
    def _compute_loss(self, model, inputs):
        ...
        self._metrics[mode]["advantages_var"].append(self.accelerator.gather(advantages).var().item())
