...
def main(script_args, training_args, model_args):
    ...
    if script_args.reward_funcs:
        script_args.reward_funcs = [func_name.strip() for func_name in script_args.reward_funcs[0].split(",")]
    ...
    if config.model_type == "qwen2":
        ...
        dataset = dataset.map(make_llama_conversation)

    # Initialize the GRPO trainer
    trainer = GRPOAhaTrainer(
        model=model,
